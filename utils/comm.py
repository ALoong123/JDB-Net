...
def generate_model(opt):
    ...
    if opt.load_ckpt is not None:
        ...
        load_ckpt_path = os.path.join('./checkpoints/'+str(opt.expID)+'/', opt.load_ckpt + '.pth')
        assert os.path.isfile(load_ckpt_path), 'No checkpoint found.'
        print('Loading checkpoint......')
        results = open('./checkpoints/' + str(opt.expID) + "/testResults.txt", "a+")
        print("\n%s checkpoint EPOCH:" % load_ckpt_path, file=results)
...
def save_gt_img(x, testset, name):
    x = x.cpu().data.numpy()
    x = np.squeeze(x)  # batch_size == 1
    x *= 255
    img_save_dir = './pred/' + testset
    ...
    if im.mode == 'F':
        im = im.convert('L')
    im.save(os.path.join(img_save_dir, name[0] + 'gt.png'))
